(() => {
 ...
 const KEY='swipe-ten.best.v1';
 let goal=10,run,current,state='home',raf=0,last=0,elapsed=0,countdown=0,questionNumber=0,drag=null,offset=0,bests={},storageOK=true,sound=false,audio=null,returnState='playing';
 ...
 function paint(){$('card').style.transform=`translate(calc(-50% + ${offset}px), -50%) rotate(${offset/18}deg)`;$('card').classList.toggle('to-left',offset < -20);$('card').classList.toggle('to-right',offset > 20);}
 function next(){const previous=current?.text;current=E.question(Math.random,previous);questionNumber++;offset=0;drag=null;$('card').hidden=false;$('expression').textContent=current.text;$('question-label').textContent=`QUESTION ${String(questionNumber).padStart(2,'0')}`;buttons(true);paint();}
 function start(){cancelAnimationFrame(raf);run=E.createRun(goal);current=null;elapsed=0;questionNumber=0;drag=null;offset=0;countdown=3000;state='countdown';show('game');$('correct').textContent='0';$('goal').textContent=`/ ${goal}`;$('timer').innerHTML='0.00<small>s</small>';$('progress').style.width='0%';$('feedback').textContent='';$('card').hidden=true;$('countdown').hidden=false;$('countdown').textContent='3';$('pause').disabled=false;buttons(false);last=performance.now();raf=requestAnimationFrame(tick);}
 ...
 function tick(now){const dt=Math.max(0,now-last);last=now;
  ...
  else if(state==='playing'){updateClock(dt);}
  if(['countdown','playing'].includes(state))raf=requestAnimationFrame(tick);
 }
 function submit(direction){if(state!=='playing')return;const now=performance.now(),dt=Math.max(0,now-last);updateClock(dt);last=now;
  const correct=E.answer(run,current,direction);drag=null;offset=0;buttons(false);$('correct').textContent=run.correct;$('progress').style.width=`${100*run.correct/run.goal}%`;$('feedback').classList.toggle('wrong',!correct);$('feedback').textContent=correct?'正解！':'おしい！';beep(correct);
  if(run.done){finish();return;}next();
 }
 ...
 function pause(){if(!['playing','countdown'].includes(state))return;const now=performance.now(),dt=Math.max(0,now-last);if(state==='playing'){updateClock(dt);}else countdown-=dt;returnState=state;state='paused';cancelAnimationFrame(raf);drag=null;offset=0;buttons(false);$('pause-dialog').showModal();}
 ...
 document.addEventListener('keydown',e=>{if(e.repeat)return;if(state==='playing'&&['ArrowLeft','ArrowRight'].includes(e.key)){e.preventDefault();submit(e.key==='ArrowRight'?'right':'left');}else if(e.key==='Escape'&&['playing','countdown'].includes(state)){e.preventDefault();pause();}});
